...
class Guru extends CI_Controller
{
	...
	public function edit($id)
	{
		...
		$data = [
			...
			'mapel'	=> $this->master->getAllMapel(),
			//'mapel'		    => $this->master->getMapelByGuru($user->email),
	...
	public function save()
	{
		...
		$this->form_validation->set_rules('nip', 'NIP', 'required|numeric|trim|min_length[8]|max_length[12]');
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($method === 'add') {
				$action = $this->master->create('guru', $input, true);
			} else if ($method === 'edit') {
