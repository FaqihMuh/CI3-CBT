...
class JenjangMapel extends CI_Controller {
	...
	public function kelas_by_jenjang($id)
	{
		$data = $this->master->getKelasByJenjang($id);
	...
	{
			//$jenjangmapel = $this->master->getJenjangMapelById($id);
		$data = [
			'user' 			=> $this->ion_auth->user()->row(),
			...
			//'jenjangmapel'	=> $this->master->getJenjangMapelByIdMapel($id),
			//'kelas'	=> $this->master->getKelasByJenjang($jenjangmapel),
			//'jenjangmapel'  => $jenjangmapel
