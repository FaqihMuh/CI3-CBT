...
class Mapel extends CI_Controller
{
	...
	public function data()
	{
		...
		if($this->ion_auth->is_admin() ){
			...
			$this->output_json($this->master->getDataMapelById($user->username), false);
		}
	...
	public function save()
	{
		...
		if ($status) {
			if ($mode == 'add') {
				...
				$this->master->createv2('mapel', $insert, 'id_mapel', null, true);
				$data['insert']	= $insert;
			} else if ($mode == 'edit') {
	...
	public function do_import()
	{
		...
		$jenjang = [];
		foreach ($data as $j) {
			$jenjang[] = ['nama_mapel' => $j];
		}

		$save = $this->master->create('mapel', $jenjang, true);
		if ($save) {
			redirect('mapel');
		} else {
