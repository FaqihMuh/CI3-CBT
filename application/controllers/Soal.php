...
class Soal extends CI_Controller {
    ...
    public function index()
	{
        ...
        if($this->ion_auth->is_admin()){
            ...
            $data['mapel'] = $this->soal->getAllMapelByGuru();
        }else{
            //Jika bukan maka mapel dipilih otomatis sesuai mapel guru
            $data['mapel'] = $this->soal->getMapelByGuru($user->username);
        }
    ...
    public function add()
	{
        ...
        if($this->ion_auth->is_admin()){
            //Jika admin maka tampilkan semua mapel
            $data['guru'] = $this->soal->getAllMapelByGuru();
        }else{
            //Jika bukan maka mapel dipilih otomatis sesuai mapel guru
            $data['guru'] = $this->soal->getMapelByGuru($user->username);
        }
    ...
    public function edit($id)
	{
        ...
        if($this->ion_auth->is_admin()){
            ...
            //Jika bukan maka mapel dipilih otomatis sesuai mapel guru
            $data['guru'] = $this->soal->getMapelKelasByGuru($soal->mapel_id);
        }
                ...
                if($key === 'file_soal'){
                    ...
                    if(!empty($_FILES[$file_abj]['name'])){    
                        if (!$this->upload->do_upload($file_abj)){
                            print($img_src.$getsoal->$file_abj);
                            $error = $this->upload->display_errors();
                            show_error($error, 500, 'File Opsi '.strtoupper($abjad[$i]).' Error');
                            exit();
                        }else{
                            if($method === 'edit'){
                                if(!empty($getsoal->$file_abj)){
