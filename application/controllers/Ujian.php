...
class Ujian extends CI_Controller {
    ...
    public function master()
	{
        ...
        $data = [
			...
			'mapel2'	=> $this->ujian->getMapelByGuruv3($user->username),
		];
	...
	public function add()
	{
		...
		$user = $this->ion_auth->user()->row();
		$mapelguru = $this->input->post('mapel', true);
		$mapelaja = $mapelguru[2];
        $data = [
			'user' 		=> $user,
			'judul'		=> 'Ujian',
			'subjudul'	=> 'Tambah Ujian',
			'mapel'	    => $this->ujian->getMapelByGuruv2($user->username)->row(),
			'mapel2'	=> $this->ujian->getMapelByGuruv3($user->username),
			'guru'		=> $this->ujian->getIdGuru($user->username),
			'mapelguru'	=> $mapelaja,
			'namamapel' => $this->master->getMapelById($mapelaja,true)
		];
	...
	public function index()
	{
		...
		$ujian 		= $this->ujian->getUjianByIdv2($id);
	...
	public function updateMapelUjian($id,$data) {
		$this->ujian->getUpdateMapelToUjian($id,$data);
	}
}