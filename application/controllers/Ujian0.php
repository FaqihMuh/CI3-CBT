...
class Ujian extends CI_Controller {
	...
	public function add()
	{
        ...
        $data = [
			...
			'mapel'     => $this->ujian->getMapelByGuruv2($user->username),
			'guru'		=> $this->ujian->getIdGuru($user->username),
		];
	...
	{
		if($id){
		$data= $this->soal->getMapelByGuru($id);
		}else{
			$data= $this->master->getAllMapel();	
		}

		$this->output->set_status_header(200)->set_content_type('application/json')->set_output(json_encode($data));
	}

	public function updateMapelGuru($id,$data) {
		$this->ujian->getUpdateMapelToGuru($id,$data);
	}
