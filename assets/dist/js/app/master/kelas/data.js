...
function load_jenjang() {
  var jenjang = $('select[name="nama_jenjang"]');
  jenjang.children("option:not(:first)").remove();

  ajaxcsrf(); // get csrf token
  $.ajax({
    url: base_url + "jenjang/load_jenjang",
    type: "GET",
    success: function(data) {
      ...
      if (data.length) {
        ...
        $.each(data, function(key, val) {
          datajenjang = `<option value="${val.id_jenjang}">${val.nama_jenjang}</option>`;
          jenjang.append(datajenjang);
        });
      }
    }
  });
