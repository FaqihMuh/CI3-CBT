function load_jenjang() {
    $('#jenjang').find('option').not(':first').remove();
    $.getJSON(base_url+'jenjang/load_jenjang', function (data) {
        var option = [];
        for (let i = 0; i < data.length; i++) {
            option.push({
                id: data[i].id_jenjang,
                text: data[i].nama_jenjang
            });
        }
        $('#jenjang').select2({
            data: option
...
function load_kelas(id) {
    ...
    $.getJSON(base_url+'kelas/kelas_by_jenjang/' + id, function (data) {
        var option = [];
        for (let i = 0; i < data.length; i++) {
            option.push({
                id: data[i].id_kelas,
